...
.controller('SpamCtrl',['$scope','apiFactory',function($scope,apiFactory){
    ...
    $scope.doAction = (content_id,action) =>{
        console.log(content_id,action);
        apiFactory.doUpdateCall('http://localhost:8080/v1/users/report/'+action+'?content_id='+content_id, {}).then(function (response) {
    ...
    $scope.getData = () =>{
        // console.log(formatDate($scope.channel.dt));
        // console.log(formatDate($scope.channel.dt2));
    
        $scope.url = 'http://localhost:8080/v1/users/spam/'+$scope.type+'?lang='+$scope.lang+'&from='+formatDate($scope.channel.dt)+'&to='+formatDate($scope.channel.dt2);
